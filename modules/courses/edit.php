<?php
// ============================================================
//  modules/courses/edit.php  —  Edit an existing course
// ============================================================
session_start();
...
$stmt->execute([$id]);
$course = $stmt->fetch();
if (!$course) { header('Location: index.php?error=Course+not+found.'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $chk = $pdo->prepare("SELECT COUNT(*) FROM courses WHERE course_code = ? AND course_id != ?");
        $chk->execute([$input['course_code'], $id]);
        if ($chk->fetchColumn() > 0) $errors[] = 'Course code already exists.';
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "UPDATE courses SET course_code = ?, course_name = ?, units = ?, description = ?
             WHERE course_id = ?"
        );
        $stmt->execute([
            $input['course_code'],
...
<head>
  ...
  <title>Edit Course — Courseware</title>
...
<body>
  ...
  <div class="page-wrap">
    ...
    <div class="page-header">
      ...
      <h2>Edit course</h2>
      <p class="course-id-hint">Editing: <strong><?= htmlspecialchars($course['course_code']) ?> — <?= htmlspecialchars($course['course_name']) ?></strong></p>
