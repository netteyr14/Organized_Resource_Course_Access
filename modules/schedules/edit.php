<?php
// ============================================================
//  modules/schedules/edit.php  —  Edit an existing schedule
// ============================================================
session_start();
...
$stmt->execute([$id]);
$schedule = $stmt->fetch();
if (!$schedule) { header('Location: index.php?error=Schedule+not+found.'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "UPDATE schedules
             SET course_id = ?, section_id = ?, day_of_week = ?,
                 time_start = ?, time_end = ?, room = ?, school_year = ?
             WHERE schedule_id = ?"
        );
        $stmt->execute([
            $input['course_id'],
...
<head>
  ...
  <title>Edit Schedule — ORCA</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-header .schedule-hint { font-size: .82rem; color: var(--muted); margin-top: .25rem; }
  ...
  <div class="page-wrap">
    ...
    <div class="page-header">
      ...
      <p class="schedule-hint">
        ...
        <?= date('g:i A', strtotime($schedule['time_start'])) ?> to
