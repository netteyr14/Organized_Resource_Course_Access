<?php
// ============================================================
//  modules/sections/edit.php  —  Edit an existing section
// ============================================================
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $input['section_name'] = trim($_POST['section_name'] ?? '');
    $input['year_id']      = (int) ($_POST['year_id'] ?? 0);

    // Validate
    if ($input['section_name'] === '') $errors[] = 'Section name is required.';
    if ($input['year_id'] === 0)       $errors[] = 'Please select a year level.';

    // Check duplicate — exclude current record
    if (empty($errors)) {
        $chk = $pdo->prepare(
            "SELECT COUNT(*) FROM sections
             WHERE section_name = ? AND year_id = ? AND section_id != ?"
        );
        $chk->execute([$input['section_name'], $input['year_id'], $id]);
        if ($chk->fetchColumn() > 0) $errors[] = 'A section with that name already exists under the selected year level.';
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "UPDATE sections SET section_name = ?, year_id = ? WHERE section_id = ?"
        );
        $stmt->execute([$input['section_name'], $input['year_id'], $id]);
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-header .section-hint { font-size: .82rem; color: var(--muted); margin-top: .25rem; }
  ...
  <div class="page-wrap">
    ...
    <div class="page-header">
      <nav class="breadcrumb-nav">
        ...
        <span>/</span>
        <a href="index.php">Sections</a>
        <span>/</span>
        <span>Edit section</span>
      </nav>
      <h2>Edit section</h2>
      <p class="section-hint">Editing: <strong><?= htmlspecialchars($section['section_name']) ?></strong></p>
      ...
      <form method="POST" action="edit.php?id=<?= $id ?>" novalidate>
        ...
        <div class="field">
          ...
          <select id="year_id" name="year_id" required>
            ...
            <?php foreach ($year_levels as $y): ?>
              <option value="<?= $y['year_id'] ?>" <?= $input['year_id'] == $y['year_id'] ? 'selected' : '' ?>>
        ...
        <div class="field">
          ...
          <input
            ...
            id="section_name"
            name="section_name"
            value="<?= htmlspecialchars($input['section_name']) ?>"
            placeholder="e.g. BSIT-1A"
            maxlength="50"
            required
            autofocus
          >
          <p class="hint">Must be unique within the selected year level.</p>
        </div>
