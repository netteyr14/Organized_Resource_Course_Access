<?php
// ============================================================
//  modules/sections/add.php  —  Add a new section
// ============================================================
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check duplicate section name under the same year level
    if (empty($errors)) {
        $chk = $pdo->prepare("SELECT COUNT(*) FROM sections WHERE section_name = ? AND year_id = ?");
        $chk->execute([$input['section_name'], $input['year_id']]);
        if ($chk->fetchColumn() > 0) $errors[] = 'A section with that name already exists under the selected year level.';
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO sections (section_name, year_id) VALUES (?, ?)");
        $stmt->execute([$input['section_name'], $input['year_id']]);
...
<head>
  ...
  <title>Add Section — Courseware</title>
